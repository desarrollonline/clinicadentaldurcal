	...
	if (orion_display_prefooter() == true) :?>
		...
		$sidebar_number = orion_get_option('prefooter-sidebars', false, '4');
		$display_prefooter = false;
		for($i = 1; $i <= $sidebar_number; $i++ ) {
			$sidebar_id = "prefooter-".$i; 
			if (is_active_sidebar( $sidebar_id )) {
				$display_prefooter = true;
			}
		}
		if ($display_prefooter == true) :
		/* Rendering */
		?>
		...
		<div class="container main-footer">
			<div class="row">
				...
				for($i = 1; $i <= $sidebar_number; $i++ ) {
					...
					$sidebar_id = "sidebar-footer-".$i; ?>
					<?php if ( is_active_sidebar($sidebar_id)) : ?>
					<div class="widgets <?php echo esc_attr($sidebar_class) . esc_attr($mobile_class);?>">
						<?php if ( is_active_sidebar($sidebar_id)) {
							dynamic_sidebar($sidebar_id);
						};?>
					</div>
