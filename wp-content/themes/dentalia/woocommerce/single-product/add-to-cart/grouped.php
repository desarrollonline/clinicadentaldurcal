...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
					...
					switch ( $column_id ) {
						case 'quantity':
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								orion_woocommerce_template_loop_add_to_cart_small();
							...
							break;
						case 'label':
							$value  = '<label for="product-' . esc_attr( $grouped_product_child->get_id() ) . '" class="h6 no-bottom-padding">';
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button btn btn-lg btn-c1 btn-download"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
